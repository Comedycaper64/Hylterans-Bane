...
public class UnitLookAtCamera : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        if (lookAtCamera)
        {
            if (invert)
            {
                Vector3 dirToCamera = (new Vector3(cameraTransform.position.x, 0, cameraTransform.position.z) - transform.position).normalized;
                transform.LookAt(transform.position + dirToCamera * -1);
            } else
            {
                transform.LookAt(new Vector3(cameraTransform.position.x, 0, cameraTransform.position.z));
            }
        }
    }
